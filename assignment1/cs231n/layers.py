...
def svm_loss(x, y):
    ...
    #### 각 이미지에 대한 정답 class 점수 배열
    correct_class_scores = x[np.arange(num_train), y]  # Shape: (N,)
    
    #### margin 값들 계산
    margins = np.maximum(0, x - correct_class_scores[:, np.newaxis] + 1)  # Shape: (N, C)
    # 정답 class score에 대해서는 loss 값을 0으로 (=고려 안함)
    margins[np.arange(num_train), y] = 0

    # loss 값 계산
    loss = np.sum(margins) / num_train  # N(데이터 개수) 평균화
